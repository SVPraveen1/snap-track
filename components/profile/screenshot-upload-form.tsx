    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          setError(`Error: ${errorJson.error}${errorJson.message ? ` - ${errorJson.message}` : ''}`)
      ...
      if (data.success) {
        ...
        setError(`Error: ${data.error || "Failed to analyze image"}`)
        throw new Error(data.error || "Failed to analyze image")
      }
    } catch (error) {
      console.error("Error analyzing image:", error)
      if (!error) {
        setError("An unknown error occurred while analyzing the image")
      }
      toast({
        title: "Error",
